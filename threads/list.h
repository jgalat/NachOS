...
class List {
  public:
    ...
    Item ItemRemove(Item item);
    Item Seek(int sortKey); // doesn't remove
...
template <class Item>
Item
List<Item>::ItemRemove(Item item)
{
    ListNode *element = first;
    ListNode *prev = element;
    
    if (IsEmpty())
      return Item();
      
    for (; element != NULL && element->item != item  ; element = element->next) {
      prev = element;
    }
...
template <class Item>
Item
List<Item>::Seek(int sortKey)
{
    ListNode *element = first;
      
    for(; element != NULL && element->key != sortKey; element = element->next);
    if (element == NULL)
      return Item();
    return element->item;
}  
