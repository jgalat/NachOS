...
Thread::Thread(const char* threadName, int _priority)
{
    ASSERT(_priority >= 0 && _priority <= THREAD_MAX_PRIORITY);
...
{
  openFiles->SortedInsert(file, fd);
  return fd++;
}
...
{
    openFiles->ItemRemove(file);
}

OpenFile*
Thread::GetFile(int _fd)
{
  return openFiles->Seek(_fd);
}
